...
namespace jsoncons
{
  ...
  template <class CharT, class ImplementationPolicy = sorted_policy, class Allocator = std::allocator<CharT>>
  class basic_json
  {
   public:
    ...
    using string_storage_type =
        typename implementation_policy::template string_storage<CharT, char_traits_type, char_allocator_type>;
    using key_storage_type =
        typename implementation_policy::template key_storage<CharT, char_traits_type, char_allocator_type>;
    ...
    using byte_string_storage_type =
        typename implementation_policy::template array_storage<uint8_t, byte_allocator_type>;

    typedef json_array<basic_json> array;

    typedef typename std::allocator_traits<allocator_type>::template rebind_alloc<key_value_pair_type>
        kvp_allocator_type;

    using object_storage_type =
        typename implementation_policy::template object_storage<key_value_pair_type, kvp_allocator_type>;
    ...
    struct variant
    {
      struct data_base
      {
        ...
        data_base( json_type_tag id ) : type_id_( id ) {}
      };

      class null_data final : public data_base
      {
       public:
        null_data() : data_base( json_type_tag::null_t ) {}
      };

      class empty_object_data final : public data_base
      {
       public:
        empty_object_data() : data_base( json_type_tag::empty_object_t ) {}
      };
      ...
      class bool_data final : public data_base
      {
       ...
       public:
        bool_data( bool val ) : data_base( json_type_tag::bool_t ), val_( val ) {}

        bool_data( const bool_data& val ) : data_base( json_type_tag::bool_t ), val_( val.val_ ) {}
      ...
      class integer_data final : public data_base
      {
       ...
       public:
        integer_data( int64_t val ) : data_base( json_type_tag::integer_t ), val_( val ) {}

        integer_data( const integer_data& val ) : data_base( json_type_tag::integer_t ), val_( val.val_ ) {}
      ...
      class uinteger_data final : public data_base
      {
       ...
       public:
        uinteger_data( uint64_t val ) : data_base( json_type_tag::uinteger_t ), val_( val ) {}

        uinteger_data( const uinteger_data& val ) : data_base( json_type_tag::uinteger_t ), val_( val.val_ ) {}
      ...
      class double_data final : public data_base
      {
       ...
       public:
        double_data( double val )
            : data_base( json_type_tag::double_t ), precision_( 0 ), decimal_places_( 0 ), val_( val )
        {
        }
      ...
      class string_data final : public data_base
      {
       ...
       public:
        string_data( const string_data& val ) : data_base( json_type_tag::string_t )
        {
          ptr_ = detail::heap_only_string_factory<char_type, Allocator>::create( val.data(), val.length(),
                                                                                 val.get_allocator() );
        }

        string_data( string_data&& val ) : data_base( json_type_tag::string_t ), ptr_( nullptr )
        {
          std::swap( val.ptr_, ptr_ );
        }

        string_data( const string_data& val, const Allocator& a ) : data_base( json_type_tag::string_t )
        {
          ptr_ = detail::heap_only_string_factory<char_type, Allocator>::create( val.data(), val.length(), a );
        }

        string_data( const char_type* data, size_t length, const Allocator& a ) : data_base( json_type_tag::string_t )
        {
      ...
      {
        typedef typename std::allocator_traits<Allocator>::template rebind_alloc<byte_string_storage_type>
            string_holder_allocator_type;
        ...
        void create( string_holder_allocator_type allocator, Args&&... args )
        {
          ...
          try
          {
            std::allocator_traits<string_holder_allocator_type>::template rebind_traits<
                byte_string_storage_type>::construct( alloc, detail::to_plain_pointer( ptr_ ),
       ...
       public:
        byte_string_data( const byte_string_data& val ) : data_base( json_type_tag::byte_string_t )
        {
          create( val.ptr_->get_allocator(), *( val.ptr_ ) );
        }

        byte_string_data( byte_string_data&& val ) : data_base( json_type_tag::byte_string_t ), ptr_( nullptr )
        {
          std::swap( val.ptr_, ptr_ );
        }

        byte_string_data( const byte_string_data& val, const Allocator& a ) : data_base( json_type_tag::byte_string_t )
        {
        ...
        ~byte_string_data()
        {
          if ( ptr_ != nullptr )
          {
            typename std::allocator_traits<string_holder_allocator_type>::template rebind_alloc<
                byte_string_storage_type>
                alloc( ptr_->get_allocator() );
            std::allocator_traits<string_holder_allocator_type>::template rebind_traits<
                byte_string_storage_type>::destroy( alloc, detail::to_plain_pointer( ptr_ ) );
            alloc.deallocate( ptr_, 1 );
          }
        }
      ...
      class array_data final : public data_base
      {
        ...
        void create( array_allocator allocator, Args&&... args )
        {
          ...
          try
          {
            std::allocator_traits<array_allocator>::template rebind_traits<array>::construct(
                alloc, detail::to_plain_pointer( ptr_ ), std::forward<Args>( args )... );
          }
          catch ( ... )
          {
            alloc.deallocate( ptr_, 1 );
       ...
       public:
        array_data( const array& val ) : data_base( json_type_tag::array_t )
        {
          create( val.get_allocator(), val );
        }

        array_data( const array& val, const Allocator& a ) : data_base( json_type_tag::array_t )
        {
          create( array_allocator( a ), val, a );
        }

        array_data( const array_data& val ) : data_base( json_type_tag::array_t )
        {
          create( val.ptr_->get_allocator(), *( val.ptr_ ) );
        }

        array_data( array_data&& val ) : data_base( json_type_tag::array_t ), ptr_( nullptr )
        {
          std::swap( val.ptr_, ptr_ );
        }

        array_data( const array_data& val, const Allocator& a ) : data_base( json_type_tag::array_t )
        {
        ...
        ~array_data()
        {
          if ( ptr_ != nullptr )
          {
            typename std::allocator_traits<array_allocator>::template rebind_alloc<array> alloc(
                ptr_->get_allocator() );
            std::allocator_traits<array_allocator>::template rebind_traits<array>::destroy(
                alloc, detail::to_plain_pointer( ptr_ ) );
            alloc.deallocate( ptr_, 1 );
          }
        }
      ...
      class object_data final : public data_base
      {
        ...
        void create( Allocator allocator, Args&&... args )
        {
          ...
          try
          {
            std::allocator_traits<object_allocator>::template rebind_traits<object>::construct(
                alloc, detail::to_plain_pointer( ptr_ ), std::forward<Args>( args )... );
          }
          catch ( ... )
          {
       ...
       public:
        explicit object_data( const Allocator& a ) : data_base( json_type_tag::object_t )
        {
          create( a, a );
        }

        explicit object_data( const object& val ) : data_base( json_type_tag::object_t )
        {
          create( val.get_allocator(), val );
        }

        explicit object_data( const object& val, const Allocator& a ) : data_base( json_type_tag::object_t )
        {
          create( object_allocator( a ), val, a );
        }

        explicit object_data( const object_data& val ) : data_base( json_type_tag::object_t )
        {
          create( val.ptr_->get_allocator(), *( val.ptr_ ) );
        }

        explicit object_data( object_data&& val ) : data_base( json_type_tag::object_t ), ptr_( nullptr )
        {
          std::swap( val.ptr_, ptr_ );
        }

        explicit object_data( const object_data& val, const Allocator& a ) : data_base( json_type_tag::object_t )
        {
        ...
        ~object_data()
        {
          if ( ptr_ != nullptr )
          {
            ...
            std::allocator_traits<Allocator>::template rebind_traits<object>::destroy(
                alloc, detail::to_plain_pointer( ptr_ ) );
            alloc.deallocate( ptr_, 1 );
          }
        }
     ...
     private:
      static const size_t data_size =
          static_max<sizeof( uinteger_data ), sizeof( double_data ), sizeof( small_string_data ), sizeof( string_data ),
                     sizeof( array_data ), sizeof( object_data )>::value;
      static const size_t data_align =
          static_max<JSONCONS_ALIGNOF( uinteger_data ), JSONCONS_ALIGNOF( double_data ),
                     JSONCONS_ALIGNOF( small_string_data ), JSONCONS_ALIGNOF( string_data ),
                     JSONCONS_ALIGNOF( array_data ), JSONCONS_ALIGNOF( object_data )>::value;
     ...
     public:
        ...
        switch ( type_id() )
        {
          ...
          case json_type_tag::integer_t:
            switch ( rhs.type_id() )
            {
              ...
              case json_type_tag::uinteger_t:
                return integer_data_cast()->value() >= 0
                           ? static_cast<uint64_t>( integer_data_cast()->value() ) == rhs.uinteger_data_cast()->value()
                           : false;
              case json_type_tag::double_t:
          ...
          case json_type_tag::uinteger_t:
            ...
            {
              case json_type_tag::integer_t:
                return rhs.integer_data_cast()->value() >= 0
                           ? uinteger_data_cast()->value() == static_cast<uint64_t>( rhs.integer_data_cast()->value() )
                           : false;
              case json_type_tag::uinteger_t:
      ...
      typename std::enable_if<std::is_pod<typename std::allocator_traits<Alloc>::pointer>::value, void>::type swap(
          variant& other ) JSONCONS_NOEXCEPT
      {
        if ( this == &other )
        {
          return;
        ...
        switch ( type_id() )
        {
          ...
          case json_type_tag::string_t:
          {
            switch ( other.type_id() )
            {
              ...
              {
                string_data temp( std::move( *string_data_cast() ) );
                new ( reinterpret_cast<void*>( &data_ ) )
                    byte_string_data( std::move( *other.byte_string_data_cast() ) );
                new ( reinterpret_cast<void*>( &other.data_ ) ) string_data( std::move( temp ) );
              }
              break;
              case json_type_tag::array_t:
    ...
    class json_proxy
    {
     ...
     public:
      ...
      template <class SAllocator>
      string_type as_string( const basic_serialization_options<char_type>& options, const SAllocator& allocator ) const
      {
      ...
      template <class T>
      typename std::enable_if<std::is_same<string_type, T>::value, T>::type as(
          const char_allocator_type& allocator ) const
      {
      ...
      void dump( std::basic_ostream<char_type>& os, const basic_serialization_options<char_type>& options,
                 bool pprint ) const
      {
      ...
      void write( std::basic_ostream<char_type>& os, const basic_serialization_options<char_type>& options,
                  bool pprint ) const
      {
      ...
      string_type to_string( const basic_serialization_options<char_type>& options,
                             char_allocator_type& allocator = char_allocator_type() ) const
      {
      ...
      void to_stream( std::basic_ostream<char_type>& os, const basic_serialization_options<char_type>& options,
                      bool pprint ) const
      {
    ...
    explicit basic_json( const Allocator& allocator ) : var_( allocator ) {}

    basic_json( const basic_json& val ) : var_( val.var_ ) {}

    basic_json( const basic_json& val, const Allocator& allocator ) : var_( val.var_, allocator ) {}

    basic_json( basic_json&& other ) JSONCONS_NOEXCEPT : var_( std::move( other.var_ ) ) {}
    ...
    basic_json( const variant& val ) : var_( val ) {}

    basic_json( variant&& other ) : var_( std::forward<variant>( other ) ) {}

    basic_json( const array& val ) : var_( val ) {}

    basic_json( array&& other ) : var_( std::forward<array>( other ) ) {}

    basic_json( const object& other ) : var_( other ) {}

    basic_json( object&& other ) : var_( std::forward<object>( other ) ) {}

    template <class ParentT>
    basic_json( const json_proxy<ParentT>& proxy ) : var_( proxy.evaluate().var_ )
    {
    }
    ...
    template <class T>
    basic_json( const T& val ) : var_( json_type_traits<basic_json, T>::to_json( val ).var_ )
    {
    }
    ...
    basic_json( const char_type* s ) : var_( s ) {}

    basic_json( const char_type* s, const Allocator& allocator ) : var_( s, allocator ) {}

    basic_json( double val, uint8_t precision ) : var_( val, number_format( precision, 0 ) ) {}

    basic_json( double val, const number_format& fmt ) : var_( val, fmt ) {}

    basic_json( const char_type* s, size_t length ) : var_( s, length ) {}

    basic_json( const char_type* s, size_t length, const Allocator& allocator ) : var_( s, length, allocator ) {}

    basic_json( const uint8_t* s, size_t length ) : var_( s, length ) {}

    explicit basic_json( const byte_string_view& s ) : var_( s.data(), s.length() ) {}

    basic_json( const uint8_t* s, size_t length, const Allocator& allocator ) : var_( s, length, allocator ) {}
#if !defined( JSONCONS_NO_DEPRECATED )
    ...
    json_proxy<basic_json> operator[]( const string_view_type& name )
    {
      switch ( var_.type_id() )
      {
        case json_type_tag::empty_object_t:
          ...
          // FALLTHRU
        case json_type_tag::object_t:
          return json_proxy<basic_json>(
              *this,
              key_storage_type( name.begin(), name.end(), char_allocator_type( object_value().get_allocator() ) ) );
          break;
        default:
      ...
      switch ( var_.type_id() )
      {
        ...
        case json_type_tag::double_t:
          handler.double_value(
              var_.double_data_cast()->value(),
              number_format( var_.double_data_cast()->precision(), var_.double_data_cast()->decimal_places() ) );
          break;
        case json_type_tag::integer_t:
    ...
    bool is_integer() const JSONCONS_NOEXCEPT
    {
      return var_.type_id() == json_type_tag::integer_t ||
             ( var_.type_id() == json_type_tag::uinteger_t &&
               ( as_uinteger() <= static_cast<uint64_t>( ( std::numeric_limits<int64_t>::max )() ) ) );
    }

    bool is_uinteger() const JSONCONS_NOEXCEPT
    {
      return var_.type_id() == json_type_tag::uinteger_t ||
             ( var_.type_id() == json_type_tag::integer_t && as_integer() >= 0 );
    }
    ...
    bool is_number() const JSONCONS_NOEXCEPT
    {
      return var_.type_id() == json_type_tag::integer_t || var_.type_id() == json_type_tag::uinteger_t ||
             var_.type_id() == json_type_tag::double_t;
    }
    ...
    typename std::enable_if<is_stateless<U>::value, void>::type create_object_implicitly()
    {
      var_ = variant( Allocator() );
    }

    template <class U = Allocator>
    typename std::enable_if<!is_stateless<U>::value, void>::type create_object_implicitly() const
    {
      JSONCONS_THROW( json_exception_impl<std::runtime_error>(
          "Cannot create object implicitly - allocator is not default constructible." ) );
    }
    ...
    void merge( const basic_json& source )
    {
      switch ( var_.type_id() )
      {
        ...
        {
          JSONCONS_THROW(
              json_exception_impl<std::runtime_error>( "Attempting to merge a value that is not an object" ) );
        }
      }
    }
    ...
    void push_back( T&& val )
    {
      switch ( var_.type_id() )
      {
        ...
        {
          JSONCONS_THROW(
              json_exception_impl<std::runtime_error>( "Attempting to insert into a value that is not an array" ) );
        }
      }
    }
  ...
  basic_json<CharT, ImplementationPolicy, Allocator> basic_json<CharT, ImplementationPolicy, Allocator>::parse(
  ...
  template <class CharT, class ImplementationPolicy, class Allocator>
  basic_json<CharT, ImplementationPolicy, Allocator> basic_json<CharT, ImplementationPolicy, Allocator>::parse(
      std::basic_istream<char_type>& is, parse_error_handler& err_handler )
  {
  ...
  class json_printable
  {
   public:
    ...
    json_printable( const Json& o, bool is_pretty_print ) : o_( &o ), is_pretty_print_( is_pretty_print ) {}

    json_printable( const Json& o, bool is_pretty_print, const basic_serialization_options<char_type>& options )
  ...
  template <class Json>
  json_printable<Json> print( const Json& val, const basic_serialization_options<typename Json::char_type>& options )
  {
